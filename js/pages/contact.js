...
function socialIcon(label) {
  ...
  return icons[label] || esc(label)
}

export default function contact() {
  var c = state.contact;

  var html = '<p class="lead" style="text-align:center;margin:28px auto 28px;max-width:560px">' + esc(c.body) + '</p>';

  html += '<div class="card contact-card" id="contactFormWrap">';
  ...
  html += '<form class="contact-form" id="contactForm" netlify name="contact">' +
    ...
    '<button class="btn primary" style="justify-content:center;width:100%">Send Message</button>' +
  '</form>';

  if (state.footer.socials && state.footer.socials.length) {
    html += '<div class="contact-socials">' +
      state.footer.socials.map(function (s) {
        return '<a href="' + esc(s[1]) + '" target="_blank" rel="noopener" aria-label="' + esc(s[0]) + '">' + socialIcon(s[0]) + '</a>'
      }).join('') + '</div>'
  }

  html += '</div>';

  return shell(esc(c.title), '', html)
}
